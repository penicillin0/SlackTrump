...
def lambda_handler(event, context):
    ...
    if 'トランプ' in text:
        print(text)
        rep = draw_trump.replace('d','♦').replace('h','♥').replace('k','♣').replace('s','♠').replace('j', 'joker')
        
        post_message_to_channel(" ーーーー\n"+'|        '+rep+'|\n'+'|                 |'+'\n'+'|'+rep+'        |\n'+" ーーーー\n"+ json.dumps(append_item))
...
def reserchTimeoutOrNot(event):
    if 'x-slack-retry-reason' in event['headers']:
        ...
        if event['headers']['x-slack-retry-reason'] in ['http_error', 'http_timeout']:
            ...
            print(event['headers']['x-slack-retry-reason'])
            return True
    return False
